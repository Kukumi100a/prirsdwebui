...
def grayscaleCUDAGPU(image):
    ...
    gray = torch.sum(image_normalized * torch.tensor([0.299, 0.587, 0.114], device='cuda'), dim=2)
    return ((gray + 1.0) * 127.5).cpu().numpy().astype(np.uint8)

def sepiaCUDAGPU(image):
    # Convert image to tensor and normalize to range [-1, 1]
    image_tensor = torch.from_numpy(image).float().cuda()
    image_normalized = (image_tensor / 255.0) * 2.0 - 1.0
    # Define sepia filter
    sepia_filter = torch.tensor([[0.393, 0.769, 0.189],
                                 [0.349, 0.686, 0.168],
                                 [0.272, 0.534, 0.131]], device='cuda').float()
    # Apply sepia filter
...
def blurCUDAGPU(image):
    # Convert image to tensor
    image_tensor = torch.from_numpy(image).permute(2, 0, 1).float().cuda().unsqueeze(0)
    
    # Apply separable blur along each channel
    blurred_channels = []
    for channel in range(3):
        channel_tensor = image_tensor[:, channel:channel+1, :, :]
        blurred_channel = F.conv2d(channel_tensor, torch.ones(1, 1, 5, 5, device='cuda') / 25.0, padding=2)
...
def contrastCUDAGPU(image, alpha=1.5, beta=0):
    # Convert image to tensor and normalize to range [-1, 1]
    image_tensor = torch.from_numpy(image).float().cuda()
    image_normalized = (image_tensor / 255.0) * 2.0 - 1.0
    # Adjust contrast
...
def edge_detectionCUDAGPU(image):
    ...
    horizontal_kernel = torch.tensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]], dtype=torch.float32, device='cuda').unsqueeze(0).unsqueeze(0)
    vertical_kernel = torch.tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]], dtype=torch.float32, device='cuda').unsqueeze(0).unsqueeze(0)
